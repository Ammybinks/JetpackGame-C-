...
namespace SpriteLibrary
{
    ...
    public class Sprite : SpriteBase
    {
        ...
        public void AccelerateDirection(double speed, double angle)
        {
            directionAngle = angle;
            double radians = MathHelper.ToRadians((float)directionAngle);   // convert current sprite angle to radians
        ...
        public void SetVelocity(double velocityX, double velocityY)
        {
            ...
            double newXSpeed = velocity.X;
            if (newXSpeed < 0)
            {
                newXSpeed *= -1;
            }

            double newYSpeed = velocity.Y;
            if (newYSpeed < 0)
            {
                newYSpeed *= -1;
            }
            ...
            if (newYSpeed > MaxSpeed) //if new speed exceeds maximum
            {
                ...
                velocity.Y *= (float)reductionFactor;    // reduce X component
            }
